...
- **FR-001**: 系统 MUST 提供一个统一的自然语言匹配 API，接收用户输入文本，返回匹配的对象和/或字段信息
...
- **FR-005**: 匹配结果 MUST 包含置信度评分，供 Agent 判断匹配质量
...
- **MatchResult**: 匹配结果集，包含对象匹配列表和字段匹配列表，每项带有置信度评分和上下文信息
- **ObjectMatch**: 单个对象匹配项，包含对象技术名、中文标题、子表列表、匹配来源（精确/同义词/标题模糊）
- **FieldMatch**: 单个字段匹配项，包含对象名+字段名、中文标题、业务类型、字段分类（金额/数量/回写/触发/虚拟/基础）
